...
import { vhToPx, hex2rgb, pxToVh, responsiveDimension } from '@/utils'
  ...
  slideRankingItemUp() {
    ...
    if (this.refGlobalRankingItemWrapper) {
      TweenMax.to(this.refGlobalRankingItemWrapper, 1.5, {
        top: responsiveDimension(this.itemWrapperPos),
...
*/
  ...
  initGlobalItemPosition(wrapperPos) {
    ...
    TweenMax.set(this.refGlobalRankingItemWrapper, {
      top: responsiveDimension(wrapperPos),
      opacity: 0,
      onComplete: () => {
...
const Points = styled.div`
  ...
  &:after {
    ...
    color: #18c5ff;
    align-self: flex-end;
    line-height: ${props => responsiveDimension(5.1)};
  }
`
...
const RankingItemsWrapper = styled.div`
  ...
  width: 100%;
  height: 100%;
  //-- 1top: ${props => responsiveDimension(100 - 15)};
  //-- 1 overflow: hidden;
  top: ${props => responsiveDimension(76.2)};
...
const rankingItemSlideUp = keyframes`
  ...
  100%{
    opacity: 1;
    transform: translateY(${responsiveDimension(-76)});
  }
`
...
const globalRankingItemSlideUp = keyframes`
  ...
  100%{
    opacity: 1;
    transform: translateY(${responsiveDimension(-85)});
  }
`
...
const GlobalSidebar = styled.div`
  ...
  left: ${props => responsiveDimension(-9)};
...
const PlacementImage = styled.div`
  ...
  background-size: contain;
  position: absolute;
  margin-top: ${props => responsiveDimension(-3)};
`
