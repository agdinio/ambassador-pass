...
export default class PrizeChestItem extends Component {
  constructor(props) {
    ...
    intercept(this.props.ProfileStore.profile.currencies, 'stars', change => {
      if (change.newValue) {
        if (this.keyword && this.keyword.match(new RegExp('starboard', 'gi'))) {
          if (this._isMounted) {
            this.setState({ stars: change.newValue })
          }
        }
      }
      return change
  ...
  animTitle() {
    const uniqueId = `${this.props.item.shortName}-${this.props.item.seasonId}${
      this.props.item.boardTypeId
    }`
...
house
  handleClick() {
    if (
      ...
      this.props.NavigationStore.addSubScreen(comp, 'StarRedeemTerms')
    } else {
      let comp = (
        <PrizeClaimTerms
          item={this.item}
