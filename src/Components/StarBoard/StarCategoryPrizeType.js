...
import { TweenMax, TimelineMax, Ease, Cubic } from 'gsap'
...
@inject('PrizeBoardStore', 'StarBoardStore', 'NavigationStore', 'ProfileStore')
export default class StarCategoryPrizeType extends Component {
  constructor(props) {
    ...
    this.disposeLocalStars = observe(
      this.props.StarBoardStore,
      'localStars',
      change => {
        if (
          !isNaN(change.newValue) &&
          !isNaN(parseInt(change.newValue)) &&
          Number.isInteger(parseInt(change.newValue))
        ) {
          if (parseInt(change.newValue) <= 0) {
            if (this._isMounted) {
              this.forceUpdate()
            }
          }
  ...
  handlePrizeItemClick(entry, userPrize) {
    this.uniqueIdentifier = `${entry.shortName}${entry.seasonId}${
      entry.boardTypeId
    }`
  ...
  handleAddStarClick(entry, e) {
    ...
    if (this.prizeForRedeem) {
      ...
      if (this.props.StarBoardStore.localStars > 0) {
        const amt = 1
        ...
        this.props.StarBoardStore.addStarLocally(entry, amt).then(response => {
          if (response) {
            ...
            this.props.StarBoardStore.debitLocalStar(amt)
  ...
  saveToServer() {
    ...
    this.props.StarBoardStore.addStar(this.entry, used).then(response => {
      if (response) {
  ...
  gotoStarRedeemTerms() {
    ...
    this.props.NavigationStore.removeSubScreen('StarBoard-AuthSequence')
  ...
  componentDidMount() {
    ...
    try {
      const imageName = evalImage(
        `starboard/${entry.shortName}-${entry.seasonId}${entry.boardTypeId}_${
          entry.images[0]
        }`
      )
      if (imageName) {
  ...
  render() {
    ...
    return (
      <Tag
        entry={entry}
        userPrize={userPrize}
        remainingStars={this.props.StarBoardStore.localStars || 0}
...
const Cons = props => {
  let icon = ''
  try {
    const imageName = `starboard/${props.entry.shortName}-${
      props.entry.seasonId
    }${props.entry.boardTypeId}_${props.entry.images[0]}`
  ...
  const uid = `${props.entry.shortName}${props.entry.seasonId}${
    props.entry.boardTypeId
  }`
  ...
  const chiefsTitle = props.entry.title.match(new RegExp('chiefs', 'gi'))
  let title = props.entry.title.toUpperCase()
  if (chiefsTitle && chiefsTitle.length > 0) {
    title = (
      <div>
        <span style={{ color: '#c61818' }}>CHIEFS</span>
        <span>
          {(
            props.entry.title.replace(new RegExp('chiefs', 'gi'), '') || ''
          ).toUpperCase()}
        </span>
      </div>
    )
      ...
      >
        ...
        >
          ...
          <ViewOrClaimButton
            ...
            font={forRedeem ? 'pamainregular' : 'pamainbold'}
            ...
            color={forRedeem ? claimColor : enoughStars ? '#ffffff' : '#ffffff'}
            onClick={props.refHandleClaimButtonClick}
            innerRef={props.refClaimButton}
            noEnoughStars={!enoughStars}
...
const ViewOrClaimButton = styled.div`
  ...
  justify-content: center;
  align-items: center;
  cursor: ${props => (props.noEnoughStars ? 'default' : 'pointer')};
  &:before {
