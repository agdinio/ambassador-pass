...
export default class GameItem extends Component {
  ...
  executeActiveItem() {
    ...
    if (refThis) {
      this.animRefs.refThisToggle = TweenMax.to(refThis, 0.3, {x: '0%', ease: new Ease(BezierEasing(0.77, 0, 0.175, 1))})
        ...
        >
          ...
          <ScheduleWrapper>
            {'live' === item.stage ? (
              ...
              <div>
                <Schedule backgroundColor={isGameEnded ? '#231F20' : GameStore.gameStatus['pregame'].bg}>
                  <PrePickSched marginLeft="8">
                    <Text font='pamainlight' size={(h/2)* 0.5} color={isGameEnded ? GameStore.gameStatus['pregame'].bg : '#ffffff'} uppercase>prepicks</Text>
                  </PrePickSched>
                  <PrePickSched marginRight="6">
                    <Text font='pamainbold' size={(h/2)* 0.5} color={isGameEnded ? 'rgba(255,255,255, 0.5)' : '#ffffff'} nospacing uppercase>{dateFormat(item.datePrePicks, 'mm/dd')}</Text>
                  </PrePickSched>
                </Schedule>
                <Schedule backgroundColor={isGameEnded ? '#231F20' : GameStore.gameStatus['live'].bg}>
                  <PrePickSched marginLeft="8">
                    <Text font='pamainlight' size={(h/2)* 0.5} color={isGameEnded ? GameStore.gameStatus['live'].bg : '#ffffff'} uppercase>start</Text>
                  </PrePickSched>
                  <PrePickSched marginRight="6">
                    <Text font='pamainbold' size={(h/2)* 0.5} color={isGameEnded ? 'rgba(255,255,255, 0.5)' : '#ffffff'} nospacing uppercase>{`${dateFormat(item.dateStart, 'mm/dd')} ${timeStart}`}</Text>
                  </PrePickSched>
                </Schedule>
              </div>
            )}
