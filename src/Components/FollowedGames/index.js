...
/*
    ...
    this.props.NavigationStore.addSubScreen(
      <Comp
        timeStart={this.handleTimeStart.bind(this, 'FollowedGames-LiveGameSchedule')}
        timeStop={this.handleTimeStop.bind(this, 'FollowedGames-LiveGameSchedule')}
      />,
          ...
          <Content>
            {followedGames && followedGames.length > 0 ? (
              followedGames.map(game => {
                return (
                  <GameItem key={`${game.sportType}-${game.gameId}`} item={game} refShareThisGame={this.handleShareThisGame.bind(this)} />
                )
              })
