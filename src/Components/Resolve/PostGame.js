...
@inject('PrePickStore', 'ProfileStore', 'ResolveStore')
@observer
export default class PostGame extends Component {
  ...
  notifyShow(response) {
    switch (response) {
      case 'postplay':
        ...
        this.props.ResolveStore.setShowPostPlay(true)
        TweenMax.to(this.PostPre, 0.3, { opacity: 0 })
        break
      case 'winstreak':
        this.showPostPlay = false
        this.props.ResolveStore.setShowPostPlay(false)
  ...
  componentDidUpdate() {
    if (!this.props.isLoading && !this.isStartedPostGame) {
      ...
      if (this.refContent) {
        TweenMax.to(this.refContent, 0.75, { top: '0%' })
      }
    }
  }
  ...
  render() {
    ...
    if (!isLoading) {
      return (
        <Container>
          <Content hasLoaded={!isLoading} innerRef={c => (this.refContent = c)}>
            <HeaderOuter>
              <Header hasLoaded={!isLoading}>
                <HeaderLeft>
                  <TextWrapper>
                    <Text
                      font={'pamainbold'}
                      size={2.5}
                      lineHeight={1.1}
                      letterSpacing={responsiveDimension(0.2)}
                      uppercase
                    >
                      winnings
                    </Text>
...
const Content = styled.div`
  ...
  top: 85%;
  position: absolute;
  animation: ${props => slideLeft} 0s forwards;
...
/*
  ${props =>
    props.hasLoaded
      ? `border-bottom: ${responsiveDimension(0.1)} solid #ffffff`
      : ``};
  }
*/
`
