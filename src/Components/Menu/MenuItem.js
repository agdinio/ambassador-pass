...
export default class MenuItem extends Component {
  ...
  render() {
    let { item } = this.props
    return (
      <Container onClick={this.route.bind(this, item.route)} id={`menu-${item.label.toLowerCase().replace(/\s+/g, '')}`}>
        <Wrapper backgroundColor={item.backgroundColor}>
          ...
          <RightWrapper bold={item.bold} color={item.labelColor} text={item.label}>
          </RightWrapper>
        </Wrapper>
      </Container>
    )
