...
export default class ChoiceIndicatorRing extends PureComponent {
  render() {
    let { teams, question, groupComponent } = this.props
    ...
    return (
      <Thumb>
        <CircleWrapper>
          {team ? (
            <TeamCircle team={team} stars={question.stars} />
          ) : (
            <BlankCircle
        ...
        <RingWrapper
          stars={question.stars}
          backgroundColor={question.stars > 0 ? StarColor : question.ringColor}
        >
          <Ring
            backgroundColor={
              question.stars > 0 ? StarColor : question.ringColor
            }
          />
        </RingWrapper>
      </Thumb>
...
const TeamCircle = props => {
  return (
    <TContainer stars={props.stars}>
      <TWrapper>
        <TTop color={props.team.iconTopColor} />
        <TBottom color={props.team.iconBottomColor} />
      </TWrapper>
      <TAbbrevWrapper>
        <TAbbrev>{props.team.teamName.charAt(0).toUpperCase()}</TAbbrev>
      </TAbbrevWrapper>
    </TContainer>
  )
}
...
const TContainer = styled.div`
  ...
  border: ${props => responsiveDimension(0.4)} solid
    ${props => (props.stars > 0 ? StarColor : '#000000')};
...
const BlankCircle = styled.div`
  ...
  background-color: ${props =>
    props.stars > 0 ? StarColor : props.backgroundColor || '#2fc12f'}
  position: absolute;
  display: flex;
  justify-content: center;
  align-items: center;
...
const RingWrapper = styled.div`
  ...
  &:before {
    ...
    position: absolute;
    opacity: 0.5;
    bottom: ${props => (props.stars > 0 ? 31 : 30)}%;
