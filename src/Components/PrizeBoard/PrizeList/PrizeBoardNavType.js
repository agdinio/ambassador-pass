...
export default class PrizeBoardNavType extends Component {
  constructor(props) {
    ...
    intercept(this.props.PrizeBoardStore, 'activeSlidingItem', change => {
      if (change.newValue) {
  ...
  handleClaimButtonClick(entry, e) {
    e.stopPropagation()
    this.reverseActiveItem()
    if (entry.qty > 0) {
...
const Grand = props => {
  let icon = ''
  try {
    const imageName = `prizeboard/${props.entry.shortName}-${
