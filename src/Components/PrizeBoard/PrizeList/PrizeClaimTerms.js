...
export default class PrizeClaimTerms extends Component {
  ...
  handleSendClick(sendType, item) {
    if (sendType === 'e-mail') {
      this.props.PrizeBoardStore.claimUserPrize(item, true).then(
        updatedUserPrize => {
          if (updatedUserPrize) {
            if (this.props.refUpdatedUserPrize) {
              this.props.refUpdatedUserPrize(updatedUserPrize)
            }
          }
        }
      )
  ...
  render() {
    ...
    let icon = ''
    try {
      const imageName = `${this.props.PrizeBoardStore.url}${item.shortName}-${
        item.seasonId
      }${item.boardTypeId}_${item.images[0]}`
          ...
          <TACContent>
            ...
            <TACSection marginTop={3}>
              {props.isClaimed ? null : (
                <AgreeButtonWrapper>
                  <AgreeButton onClick={props.refAgreeClick}>
