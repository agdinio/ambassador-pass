...
@inject('PrizeBoardStore', 'NavigationStore', 'ProfileStore', 'StarBoardStore')
@observer
export default class PrizeBoard extends Component {
  ...
  handleLoggedIn() {
    ...
    let comp = <StarBoard refHideBanner={this.handleHideBanner.bind(this)} isSubScreen key={`prizeboard-starboard-1`} />
  ...
  MenuOptions(currentItem) {
    ...
    if ('bb' === currentItem.toLowerCase()) {
    ...
    } else if ('sr' === currentItem.toLowerCase()) {
      // comp = (
      //   <StarBoardFile
      //     profile={this.props.ProfileStore.profile}
      ...
      // }
      this.bannerType = 'StarBoard'
      comp = <StarBoard refHideBanner={this.handleHideBanner.bind(this)} isSubScreen key={`prizeboard-starboard-2`} />
      this.props.NavigationStore.addSubScreen(comp, 'StarBoard')
    } else {
          ...
          return (
            this.props.PrizeBoardStore.prizes.filter(secondary => {
              return (
                ...
                !excludedItems.match(new RegExp(primary.id, 'gi')) &&
                !primary.id.match(new RegExp('sr', 'gi'))
              )
            }).length > 0
          )
        }
              ...
              <BoardsInner innerRef={ref => (this.RefBoardsInner = ref)}>
                {prizeBoards.map((menu, key) => {
                  const details = prizes.filter(
                    o =>
                      o.boardTypeId.toLowerCase() === menu.id.toLowerCase() &&
                  ...
                  if (
                    (details &&
                      details.length > 0 &&
                      !menu.id.match(new RegExp('sr', 'gi'))) ||
                    menu.id.toUpperCase() === 'SR'
                  ) {
