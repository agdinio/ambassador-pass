      ...
      <Container>
        {
          GameStore.PlayTypes.map(playType => {
            ...
            if (GameStore.gameHistory && GameStore.gameHistory.livePlays && Array.isArray(GameStore.gameHistory.livePlays)) {
              const resolved = GameStore.gameHistory.livePlays.filter(o => o.type.toLowerCase() === playType.type.toLowerCase())[0]
              if (resolved) {
            ...
            return (
              <PlayContainer key={`playbar-${playType.type}`}>
                <PlayBar backgroundColor={playType.backgroundColor}>
                  ...
                  <PlayIcon src={evalImage(playType.icon)} borderColor={playType.backgroundColor} iconBG={playType.iconBG} />
                </PlayBar>
                {
                  'stars'.match(new RegExp(playType.type,'gi')) ? (
                    ...
                    <PlayStats>
                      <Text font="pamainbold" size="4.6" color={totalPlays < 1 ? 'rgba(255,255,255,0.5)' : totalCorrectAnswers < totalPlays ? 'rgba(255,255,255,0.5)' : playType.backgroundColor}>{totalCorrectAnswers}</Text>
                      <Text font="pamainlight" size="4.6" color={totalPlays > 0 && totalCorrectAnswers === totalPlays ? playType.backgroundColor : '#ffffff'} style={{marginLeft:'10%', marginRight:'5%', marginTop:responsiveDimension(0.2)}}>/</Text>
                      <Text font="pamainbold" size="4.6" color={totalPlays < 1 ? 'rgba(255,255,255,0.5)' : totalCorrectAnswers < totalPlays ? '#ffffff' : playType.backgroundColor}>{totalPlays}</Text>
                    </PlayStats>
                  )
                }
              </PlayContainer>
...
const PlayContainer = styled.div`
  ...
  margin: ${props => responsiveDimension(0.2)} 0 ${props => responsiveDimension(0.2)} 0;
...
const PlayIcon = styled.div`
  ...
  border-radius: 50%;
  border: ${props => `${responsiveDimension(barHeight * 0.08)} solid ${props.borderColor}`};
