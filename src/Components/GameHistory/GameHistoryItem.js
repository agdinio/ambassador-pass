...
export default class GameHistoryItemItem extends Component {
  ...
  executeActiveItem() {
    ...
    if (refThis) {
      this.animRefs.refThisToggle = TweenMax.to(refThis, 0.2, {x: '0%', ease: new Ease(BezierEasing(0.77, 0, 0.175, 1))})
  ...
  handleViewPlayHistoryClick(e) {
    ...
    this.props.NavigationStore.addSubScreen(<Comp item={this.props.item} />, 'GameHistory-GameHistoryView')
  }
    ...
    return (
      <Container>
        ...
        >
            ...
            <TeamRowWrap>
              {
                participants.map(participant => {
                  return (
                    <TeamRow key={`participant-${participant.participantId}`} backgroundColor={participant.backgroundColor}>
                      <TeamIconWrapper>
                        <TeamIcon
                        ...
                        />
                        <TeamName color={participant.teamNameColor}>{participant.name}</TeamName>
                      </TeamIconWrapper>
                      <TeamScore
          ...
          <AnalyticsWrapper>
            <AnalyticsRow>
              <Text font='pamainlight' size="2.8" color={'#17c5ff'} nospacing uppercase>pts</Text>
              <Text font='pamainbold' size="2.8" color={'#17c5ff'} nospacing uppercase>{item.pointsEarned}</Text>
            </AnalyticsRow>
            <AnalyticsRow>
              <Text font='pamainlight' size="2.8" color={'#ffffff'} nospacing uppercase>w / l</Text>
              <Text font='pamainbold' size="2.8" color={'#ffffff'} uppercase>60%</Text>
            </AnalyticsRow>
            <AnalyticsRow>
              <Text font='pamainlight' size="2.8" color={'#edbf00'} nospacing uppercase>used</Text>
              <Text font='pamainbold' size="2.8" color={'#edbf00'} nospacing uppercase>{item.tokensUsed}</Text>
            </AnalyticsRow>
          </AnalyticsWrapper>

          <SlidingPanel innerRef={ref => this[`refSlidingPanel-${item.gameId}`] = ref}>
            <SlidingSection widthInPct="60" />
            <SlidingSection widthInPct="40">
              <ButtonViewPlayHistory onClick={this.handleViewPlayHistoryClick.bind(this)} />
            </SlidingSection>
          </SlidingPanel>
...
const SlidingButton = styled.div`
  ...
  ${props => props.hasBorder ? `border:${responsiveDimension(0.4)} solid ${props.color}` : ``};
  display: flex;
  justify-content: center;
  align-items: center;
  cursor: pointer;
  ${props => props.marginRight ? `margin-right:${responsiveDimension(props.marginRight)}` : ``};
  &:after {
    content: '${props => props.text}';
    font-family: pamainbold;
...
const AnalyticsRow = styled.div`
  ...
  justify-content: space-between;
  align-items: center;
  padding: ${props => `${responsiveDimension(0.5)} ${responsiveDimension(2)} ${responsiveDimension(0.5)} ${responsiveDimension(2)}`};
`
