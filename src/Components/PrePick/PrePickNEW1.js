...
@inject('PrePickStore', 'NavigationStore', 'ProfileStore', 'LiveGameStore')
@withRouter
export default class PrePickNEW1 extends Component {
  constructor(props) {
    super(props)
    this.lastRow = null
    this.state = {
  ...
  assembleQuestion() {
    ...
    let question = PrePickStore.questions.filter(o => o.prepickSequence === PrePickStore.currentPrePick)[0]
    if (question) {
      question.labels.sort((a, b) => a.sequence > b.sequence)
    }

    let message = PrePickStore.messages.filter(o => o.prepickSequence === PrePickStore.currentPrePick)[0]
    if (message) {
      message.headers.sort((a, b) => a.sequence > b.sequence)
      message.details.sort((a, b) => a.sequence > b.sequence)
    }

    const prepickBackground = require(`@/assets/images/${question.background}`)
    util.loadImagesSelectedUponPageLoad([prepickBackground], next => {
      if (next) {
        this.setState({question: question, message: message})
        if (this.refBGContainer) {
          this.refBGContainer.style.backgroundImage = 'url(' + prepickBackground + ')'
        }
        this.fadeInQuestion()
      }
    })
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (!util.isEqual(this.state.question, nextState.question)) {
      return true
    }

    if (this.state.notifyPicksPointsTokens !== nextState.notifyPicksPointsTokens) {
      return true
    }
  ...
  componentDidMount() {
    ...
    this.props.PrePickStore.captureAnalyticPrePickStart()
    this.props.LiveGameStore.assignGUIDToPlays()
  }
  ...
  render() {
    ...
    let SliderTag = question ? question.choiceType === 'AB' ? ABSlider : MultiSelectSlider : null
...
const QuestionPanelWrapper = styled.div`
  padding-top: ${util.vhToPx(2.9)};
  text-align: center;
`
const SliderWrapper = styled.div`
  padding-top: ${util.vhToPx(3)};
`
...
const NextPlayType = styled.div`
  ...
  align-items: center;
  width: 14%;
  height: ${util.vhToPx(4.6)};
  border-radius: 0 ${util.vhToPx(4.6)} ${util.vhToPx(4.6)} 0;
  background-color: #146314;
  border-top: ${util.vhToPx(0.3)} solid #2fc12f;
  border-right: ${util.vhToPx(0.3)} solid #2fc12f;
  border-bottom: ${util.vhToPx(0.3)} solid #2fc12f;
  padding-right: ${util.vhToPx(0.1)};
  overflow: hidden;
...
const PlayIcon = styled.div`
  position: absolute;
  width: ${util.vhToPx(3.8)};
  height: ${util.vhToPx(3.8)};
  overflow: hidden;
  border-radius: 100%;
  background-color: #2fc12f;
  &:before {
