  ...
  evaluateTotalPoints() {
    ...
    let check = setInterval(() => {
      ...
      if (this.props.ProfileStore.profile.currencies['points'] >= currPts) {
        ...
        this.props.ProfileStore.profile.currencies['points'] =
          currPts - this.props.question.points
        //ORIG
  ...
  evaluateTotalTokens() {
    ...
    let check = setInterval(() => {
      ...
      if (this.props.ProfileStore.profile.currencies['tokens'] >= currTks) {
        ...
        //TEMPORARY - CODE_SERVER.JS
        this.props.ProfileStore.profile.currencies['tokens'] =
          currTks - this.props.question.tokens
        //ORIG
...
*/
    ...
    intercept(this.props.CommonStore, 'tokens', change => {
      if (change.newValue) {
        if (this.refs.refTotalWrapper) {
          TweenMax.to(ReactDOM.findDOMNode(this.refs.refTotalWrapper), 0.3, {
            scale: 1.2,
            transformOrigin: 'right',
          })
        }

        let toAdd = change.newValue
        let finalTokens = change.newValue + this.totalTokens
        ...
        this.check = setInterval(() => {
          if (this.totalTokens < finalTokens) {
            ...
            if (this.refs.refTotalWrapper) {
              TweenMax.to(
                ReactDOM.findDOMNode(this.refs.refTotalWrapper),
                0.3,
                { scale: 1 }
              )
            }
  ...
  render() {
    ...
    return (
      <TotalWrapper innerRef={reference} ref="refTotalWrapper">
        <TotalPointsWrapper>
...
export const StaticPointsAndTokens = props => {
  return (
    <TotalWrapper innerRef={props.reference}>
      <TotalPointsWrapper>
        <TotalPointsValue>{Math.floor(props.totalPoints)}</TotalPointsValue>
...
const ContentWrapper = styled.div`
  ...
  bottom: 0;
  width: 100%;
  padding-bottom: ${props => responsiveDimension(1.5)};
`
...
const Points = styled.div`
  color: #fff;
  font-size: ${responsiveDimension(5.7)};
  font-family: pamainbold;
  &:after {
    content: 'PTS';
    color: #18c5ff;
    font-family: pamainlight;
    font-size: ${responsiveDimension(5.3)};
    margin-left: ${responsiveDimension(0.5)};
    display: inline-block;
  }

  position: absolute;
  top: ${props => props.top}%;
  right: ${responsiveDimension(3)};
