...
*/
  ...
  @action
  setCurrentMainQuestion(nextQuestionId) {
    this.currentMainQuestion = paScript.filter(o => o.id === nextQuestionId)[0]
  }

  @action
  setPreviousScript(curr) {
    //this.previousScriptItem = paScript.filter(o => o.id === curr)[0]
    this.previousScriptItem = paScript.filter(o => o.nextId === curr)[0]
  }
  ...
  getCurrentScriptItem(prev, nextId) {
    ...
    if (nextId) {
      this.setPreviousScript(nextId)
      this.setNextScript(nextId)
    }

    if (this.currentScriptItem.advantage) {
      if (!this.scorings[this.currentScriptItem.id]) {
        this.scorings[this.currentScriptItem.id] = this.currentScriptItem.advantage.score
        const team = PrePickStore.teams.filter(o => o.id === this.currentScriptItem.advantage.teamId)[0]
        if (team) {
          team.score += this.currentScriptItem.advantage.score
        }
      }
    }


    return this.currentScriptItem
  }
  ...
  resetVars() {
    ...
    this.currentMainQuestion = {}
    this.currentScriptItem = {}
    this.previousScriptItem = {}
    this.inProgress = false
  }

  assignGUIDToPlays() {
    for (let i = 0; i < paScript.length; i++) {
      paScript[i].guid = guid()
    }
  }
  ...
  assignIdToPlays() {
    return new Promise(resolve => {
      for (let i = 0; i < paScript.length; i++) {
        const currId = paScript[i].id
        //const newId = new Date().getTime().toString().substring(6)
        const newId = guid()
        this.assignNexPlayId(currId, newId)
        this.assignIdToSponsorLogos(currId, newId)
        this.assignIdToResults(currId, newId)
        this.assignIdToTextCard(currId, newId)
  ...
  assignNexPlayId(currNextId, newNextId) {
    for (let j = 0; j < paScript.length; j++) {
      let pa = paScript[j]

      if (pa.nextId === currNextId) {
        pa.nextId = newNextId
      }

      if (pa.choices && pa.choices.length > 1) {
        for (let x = 0; x < pa.choices.length; x++) {
          if (pa.choices[x].nextId === currNextId) {
            pa.choices[x].nextId = newNextId
          }
        }
      }
    }
  ...
  assignIdToSponsorLogos(currId, newId) {
    for (let k = 0; k < this.sponsorLogos.length; k++) {
      let logo = this.sponsorLogos[k]
      if (logo.id && logo.id === currId) {
        logo.id = newId
      }
    }
  }

  assignIdToResults(currId, newId) {
    for (let m = 0; m < this.resultsScript.length; m++) {
      let scr = this.resultsScript[m]
      if (scr.scriptId && scr.scriptId === currId) {
        scr.scriptId = newId
      }
    }
  }

  assignIdToTextCard(currId, newId) {
    for (let n = 0; n < this.textCardScript.length; n++) {
      let scr = this.textCardScript[n]
      if (scr.scriptId && scr.scriptId === currId) {
        scr.scriptId = newId
      }
    }
  ...
  videoPrevTime = 0
  @action
  setVideoPrevTime(val) {
    this.videoPrevTime = val
  }

  minutesToSeconds(input) {
