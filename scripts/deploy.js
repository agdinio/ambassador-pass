...
const FtpCredentials = {
  ambPass: {
    host: 'ftp.dennisonmediagroup.com', // 'sportocoplayalong.com/reviewnew1/',
    user: 'Udbeb83j@reviewnew1.sportoco.com', // 'dev54786yiuhkj@sportocoplayalong.com',
    password: '73nejeixbJdDnr!', // 'u987t6rfyGV!Bhy807'
  },
}

let credentials = FtpCredentials.ambPass

run('ls', ['-al'], { cwd: path.join(__dirname, '..') })
  .then(_ => run('ls', ['-al'], { cwd: path.join(__dirname, '..', 'build') }))
  .then(_ => {
    let list = walkSync(BuildDir)
    console.log(`Uploading compiled web app to the reviewnew1 site`)
